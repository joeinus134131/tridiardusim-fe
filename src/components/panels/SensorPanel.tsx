    ...
    <div className="absolute left-4 top-20 w-64 glass-panel flex flex-col z-[var(--z-panel)] shadow-2xl animate-slide-right border-[var(--border)] max-h-[60vh] overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
        {sensors.map((sensor) => (
          <div
            key={sensor.id}
            className="bg-[var(--bg-elevated)] p-3 rounded-lg border border-[var(--border)]"
          >
            ...
            {sensor.typeId === "potentiometer" && (
              <div>
                <div className="flex justify-between text-xs text-[var(--text-muted)] mb-1">
                  <span>0</span>
                  <span>
                    {Math.floor(Number(sensor.state?.value || 0) * 1023)}
                  </span>
                  <span>1023</span>
                </div>
                <input
                  ...
                  onChange={(e) =>
                    updateComponentState(sensor.id, {
            ...
            {sensor.typeId === "push_button" && (
              <button
                ...
                onMouseDown={() =>
                  updateComponentState(sensor.id, { isPressed: true })
                }
                onMouseUp={() =>
