...
type Expr =
  ...
  | { kind: "assign"; name: string; op: string; value: Expr }
  | { kind: "post"; name: string; op: string };
type Stmt =
  | { kind: "block"; body: Stmt[] }
  | { kind: "declare"; name: string; value: Expr }
  | { kind: "expr"; value: Expr }
  | { kind: "if"; test: Expr; yes: Stmt; no?: Stmt }
  | { kind: "while"; test: Expr; body: Stmt }
  | { kind: "for"; init: Stmt; test: Expr; step: Expr; body: Stmt }
...
export class SketchParser {
  private tokens: { text: string; line: number }[] = [];
  private i = 0;
  functions = new Map<string, Fn>();
  globals: Stmt[] = [];
  constructor(source: string) {
    ...
    while (this.peek() !== "<eof>") {
      ...
      if (this.eat("(")) {
        ...
        this.need(";");
        this.globals.push({ kind: "declare", name, value });
      }
    }
    if (!this.functions.has("setup") || !this.functions.has("loop"))
      this.fail("Diperlukan void setup() dan void loop()");
  }
  private peek() {
    return this.tokens[this.i]?.text || "<eof>";
  }
  ...
  private fail(message: string): never {
    throw new Error(
      `Baris ${this.tokens[this.i]?.line}: ${message}; ditemukan '${this.peek()}'.`,
    );
  }
  private need(s: string) {
    if (!this.eat(s)) this.fail(`Diharapkan ${s}`);
  }
  private name() {
    const n = this.peek();
    if (!/^[A-Za-z_]\w*$/.test(n)) this.fail("Diharapkan nama");
    this.i++;
    return n;
  }
  ...
  private block(): Stmt {
    ...
    const body: Stmt[] = [];
    while (this.peek() !== "}") {
      if (this.peek() === "<eof>") this.fail("Blok belum ditutup");
      body.push(this.stmt());
    }
    this.need("}");
  ...
  private stmt(): Stmt {
    ...
    if (this.eat("if")) {
      ...
      this.need(")");
      const yes = this.stmt();
      const no = this.eat("else") ? this.stmt() : undefined;
    ...
    if (this.eat("while")) {
      ...
      const test = this.expr();
      this.need(")");
      return { kind: "while", test, body: this.stmt() };
    }
    if (this.eat("for")) {
      this.need("(");
      ...
      this.need(")");
      return { kind: "for", init, test, step, body: this.stmt() };
    }
    if (this.eat("return")) {
      const value = this.peek() === ";" ? undefined : this.expr();
      this.need(";");
  ...
  private expr(min = 1): Expr {
    ...
    if (["!", "-", "+", "~", "&", "*"].includes(t))
      ...
      left = {
        kind: "literal",
        value: t.startsWith("'") ? s.charCodeAt(0) : s,
      };
    } else if (/^[A-Za-z_]\w*$/.test(t)) {
      let name = t;
      if (this.eat(".")) name += "." + this.name();
      if (this.eat("(")) {
        const args: Expr[] = [];
        if (this.peek() !== ")")
          do {
    ...
    if (this.peek() === "++" || this.peek() === "--") {
      if (left.kind !== "name") this.fail("Increment membutuhkan variabel");
      left = { kind: "post", name: left.name, op: this.peek() };
      this.i++;
    }
    while ((precedence[this.peek()] || 0) >= min) {
      const op = this.peek();
      this.i++;
      const right = this.expr(precedence[op] + (precedence[op] === 1 ? 0 : 1));
      if (precedence[op] === 1) {
        if (left.kind !== "name") this.fail("Assignment membutuhkan variabel");
        left = { kind: "assign", name: left.name, op, value: right };
...
class Scope {
  ...
  get(n: string): Value {
    ...
    throw new Error(`Nama tidak dikenal: ${n}`);
  }
  set(n: string, v: Value): Value {
    if (this.values.has(n)) {
      this.values.set(n, v);
      return v;
    }
    if (this.parent) return this.parent.set(n, v);
    throw new Error(`Variabel belum dideklarasikan: ${n}`);
  }
}
class ReturnValue {
...
export class SketchRuntime {
  ...
  private depth = 0;
  constructor(
    private parser: SketchParser,
    private api: Record<string, (...args: Value[]) => Value | Promise<Value>>,
  ...
  private tick() {
    if (++this.steps > 50000)
      throw new Error(
        "Batas 50.000 operasi tercapai tanpa delay/akhir loop. Periksa loop tak berujung.",
      );
  }
  resetBudget() {
    this.steps = 0;
  ...
  async start() {
    for (const s of this.parser.globals) await this.stmt(s, this.scope);
  ...
  private async call(name: string, args: Value[]): Promise<Value> {
    ...
    fn.params.forEach((p, i) => scope.values.set(p, args[i] ?? 0));
    try {
      await this.stmt(fn.body, scope);
    } catch (e) {
  ...
  private async expr(e: Expr, s: Scope): Promise<Value> {
    ...
    switch (e.kind) {
      ...
      case "post": {
        const v = Number(s.get(e.name));
        s.set(e.name, v + (e.op === "++" ? 1 : -1));
        return v;
      }
      case "assign": {
        const v = await this.expr(e.value, s);
        return s.set(
          e.name,
          e.op === "=" ? v : this.binary(e.op[0], s.get(e.name), v),
        );
      }
      case "binary": {
        const l = await this.expr(e.left, s);
        if (e.op === "&&" && !l) return 0;
        if (e.op === "||" && l) return 1;
        return this.binary(e.op, l, await this.expr(e.right, s));
      }
    }
  }
  private binary(op: string, l: Value, r: Value): Value {
    ...
    switch (op) {
      ...
      case "*":
        return a * b;
      case "/":
        if (!b) throw new Error("Pembagian nol");
        return a / b;
      case "%":
        return a % b;
      case "<":
    ...
    throw new Error(`Operator tidak didukung: ${op}`);
  }
  private async stmt(st: Stmt, s: Scope): Promise<void> {
    ...
    switch (st.kind) {
      case "block": {
        ...
        for (const x of st.body) await this.stmt(x, child);
        break;
      }
      case "declare":
        s.values.set(st.name, await this.expr(st.value, s));
        break;
      case "expr":
        await this.expr(st.value, s);
        break;
      case "if":
        if (await this.expr(st.test, s)) await this.stmt(st.yes, s);
        else if (st.no) await this.stmt(st.no, s);
        break;
      case "return":
        throw new ReturnValue(st.value ? await this.expr(st.value, s) : 0);
      case "break":
        throw new BreakLoop();
      case "while":
        while (await this.expr(st.test, s)) {
          try {
            await this.stmt(st.body, s);
          } catch (e) {
      ...
      case "for": {
        const child = new Scope(s);
        await this.stmt(st.init, child);
        while (await this.expr(st.test, child)) {
          try {
            await this.stmt(st.body, child);
          } catch (e) {
            if (e instanceof BreakLoop) break;
            throw e;
          }
          await this.expr(st.step, child);
        }
        break;
      }
    }
